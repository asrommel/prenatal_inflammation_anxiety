...
# ==============================================================================
# 0B. VALIDATE DATA FILES EXIST
# ==============================================================================

required_files <- c(
  "Mental health hx 2024-6-6_all variables.csv",
  "GenC20-CarlyGADAndImmuneAct_DATA_2024-07-15_1505_FM.csv",
  "Logbook 4.20.21_ML2 1.20.23 Frederieke_[55].xlsx - Sheet 1 - Logbook 4.20.21_ML2 1 (1).csv"
)
...
if (!all_files_present) {
  stop(
    ...
    "Please place the following files in the '", data_dir, "/' directory:\n",
...
mental_health_raw <- read.csv(
  file.path(data_dir, "Mental health hx 2024-6-6_all variables.csv")
)
cat("✓ Loaded mental health history data\n")

data <- read.csv(
  file.path(data_dir, "GenC20-CarlyGADAndImmuneAct_DATA_2024-07-15_1505_FM.csv")
)
cat("✓ Loaded main study dataset\n")

logbook <- read.csv(
  file.path(data_dir, "Logbook 4.20.21_ML2 1.20.23 Frederieke_[55].xlsx - Sheet 1 - Logbook 4.20.21_ML2 1 (1).csv")
)
cat("✓ Loaded sample logbook\n")
...
pdf(file.path(output_dir, "Supplemental_Figure_S1_Timing_Distributions.pdf"),
    width = 12, height = 4)
...
cat("✓ Multiple imputation completed (50 datasets)\n")
...
print(table1_ft)
save_as_html(table1_ft, path = file.path(output_dir, "Table_1_Participant_Characteristics_by_GAD7_Percentile_Group.html"))
save_as_docx("Table 1" = table1_ft, path = file.path(output_dir, "Table_1.docx"))
...
print(table2_gt)
gtsave(table2_gt, file.path(output_dir, "Table_2.docx"))
...
flowchart %>%
  export_svg() %>%
  charToRaw() %>%
  rsvg_pdf(file.path(output_dir, "Figure_1_Flowchart_of_Study_Sample.pdf"))

flowchart %>%
  export_svg() %>%
  charToRaw() %>%
  rsvg_png(file.path(output_dir, "Figure_1_Flowchart_of_Study_Sample.png"), width = 2400, height = 1800)

flowchart

cat("✓ Created Figure 1 (Flowchart)\n")
...
ggsave(
  file.path(output_dir, "Figure_2_forest_plot_gad7.pdf"),
  plot = forest_plot,
  width = 7,
  height = 5,
  device = cairo_pdf
...
ggsave(
  file.path(output_dir, "Figure_2_forest_plot_gad7.png"),
  plot = forest_plot,
  width = 7,
  height = 5,
  dpi = 300
...
gtsave(
  supp_table_s1,
  file.path(output_dir, "Supplemental_Table_S1_Full_Multivariable_Quantile_Regression_Results.docx")
)
...
write.csv(
  ...
  file.path(output_dir, "Supplemental_Table_S1_Full_Multivariable_Quantile_Regression_Results.csv"),
  row.names = FALSE
)
...
cat("\nGenerated files:\n")
cat("  Figures:\n")
cat("    - Figure_1_Flowchart_of_Study_Sample.pdf\n")
cat("    - Figure_2_forest_plot_gad7.pdf\n")
cat("    - Supplemental_Figure_S1_Timing_Distributions.pdf\n")
cat("    - Supplemental_Figure_S2_Collinearity.pdf\n")
cat("  Tables:\n")
cat("    - Table_1.docx\n")
cat("    - Table_2.docx\n")
